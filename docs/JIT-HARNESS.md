...
This package expresses **Requesting Code Review** as a typed, task-adaptive agent harness. Its concrete capability remains: Run a pre-commit quality and security pass and prepare a focused review request. The harness contract makes memory, planning, action, capability selection, failure handling, and verification explicit.
...
| Module | Strategy | Responsibility in Requesting Code Review |
| --- | --- | --- |
| Memory | `stage_artifact_log` | Retain local changes plus decisions, observations, and verification evidence needed to resume safely. |
| Planning | `linear_gated_plan` | Transform security scan into the ordered, bounded stages defined by this skill. |
...
| Action | `inspect_change_test_handoff` | Advance the workflow through quality checks while preserving stop conditions and user authority. |
