<p align="center">
  <img src="assets/system-map.svg" alt="Detailed system map for Requesting Code Review" width="100%">
</p>
...
Technical work becomes difficult to review when discovery, decisions, changes, and verification are mixed together. **Requesting Code Review** turns that work into an explicit sequence with visible inputs, outputs, review points, and completion evidence.

## Why the repository has this name

The shared `hermes-skill-` prefix identifies this as a portable Hermes workflow package. `requesting-code-review` names the capability directly—requesting code review—so the repository remains searchable and understandable outside the original AI-OS workspace. The public title is **Requesting Code Review**.
...
| What does it do? | Run a pre-commit quality and security pass and prepare a focused review request. |
...
![Requesting Code Review system map](assets/system-map.svg)

## Operation sequence

![Requesting Code Review actor and data sequence](assets/operation-sequence.svg)
...
![Illustrative output produced by Requesting Code Review](assets/example-output.svg)
...
> Use a disposable sample repository to run a pre-commit quality and security pass and prepare a focused review request. Return the result, the evidence used to verify it, and any limitations or actions that still require approval.
